...
def draw_textbox(self, img, current_entitiy, location):
    ...
    draw.text((x1 + current_entitiy.text_margin, y1 + h + current_entitiy.text_margin), label,
              font=current_entitiy.font, fill=current_entitiy.text_color, anchor='lb')
    img = np.array(img_pil)

    bbox = [[x1 + current_entitiy.text_margin, y1 + current_entitiy.text_margin],
            [x1 + current_entitiy.text_margin + w, y1 + current_entitiy.text_margin + h]]
...
def get_spline_anchors(self, entity1_center, entity2_center, entity1_bbox, entity2_bbox, entity_configuration):
    ...
    for idx in range(canidate_points.shape[0] - 1):
        ...
        if current_point[0] < entity1_bbox[0][0] or current_point[1] < entity1_bbox[0][1] or current_point[0] > \
                entity1_bbox[1][0] or current_point[1] > entity1_bbox[1][1]:
            count += 1
            if count == tar_point:
    ...
    for idx in range(canidate_points.shape[0] - 1, 0, -1):
        current_point = canidate_points[idx, :]
        if current_point[0] < entity2_bbox[0][0] or current_point[1] < entity2_bbox[0][1] or current_point[0] > \
                entity2_bbox[1][0] or current_point[1] > entity2_bbox[1][1]:
            count += 1
            if count == tar_point:
    ...
    if entity_configuration == LONG or entity_configuration == TALL:
        ...
        if rand_int == 0 and abs(entity1_center[0] - entity2_center[0]) > 50 and abs(
                entity1_center[1] - entity2_center[1]) > 50:
            self.spline_type = CORNER
            spline_points = get_corner_anchors(entity_configuration, entity1_center, entity2_center, entity1_bbox,
                                               entity2_bbox)
        elif rand_int == 1:
            # ARCH
            spline_points = get_arch_anchors(self, start_point, end_point)
...
def get_square(self, a, b):
    ...
    rows, cols = draw.rectangle_perimeter(tuple([0, 0]),
                                          tuple([a + (self.text_margin * 2), b + (self.text_margin * 2)]))
    rows = rows.flatten()
    cols = cols.flatten()
...
def draw_cluster(entity1, current_entitiy, placed_entities, img):
    ...
    for entity in placed_entities:
        cluster_mask = entity.draw_shape(cluster_mask.astype(np.uint8),
                                         [entity.center[0] + 500, entity.center[1] + 500], (255, 255, 255),
                                         (255, 255, 255))
    ...
    while True:
        ...
        test_center = [int(shape1_x[tmp_idx] * factor) + entity1_center[0],
                       int(shape1_y[tmp_idx] * factor) + entity1_center[1]]
        new_entity_mask = current_entitiy.draw_shape(new_entity_mask.astype(np.uint8),
                                                     [test_center[0] + 500, test_center[1] + 500], (255, 255, 255),
                                                     (255, 255, 255))
    ...
    new_center = [int(shape1_x[tmp_idx] * factor) + entity1_center[0],
                  int(shape1_y[tmp_idx] * factor) + entity1_center[1]]

    return new_center, img


def draw_relationship(self, img, entity1_center, entity2_center, entity_configuration, placed_entities1,
                      placed_entities2, text1_shape, text2_shape):
    """
    ...
    for idx in range(len(placed_entities1)):
        ...
        current_entitiy.textbox_background = (
        np.random.randint(0, 255), np.random.randint(0, 255), np.random.randint(0, 255))
        current_entitiy.text_color = (
        255 - current_entitiy.textbox_background[0], 255 - current_entitiy.textbox_background[1],
        255 - current_entitiy.textbox_background[2])

        current_center = [current_entitiy.center[0] + entity1_center[0] - int(w1 / 2),
                          current_entitiy.center[1] + entity1_center[1] - int(h1 / 2)]
    ...
    w1 = text1_shape[0]
    h1 = text1_shape[1]
    entity1_bbox = [[int(entity1_center[0] - (w1 / 2)), int(entity1_center[1] - (h1 / 2))],
                    [int(entity1_center[0] + (w1 / 2)), int(entity1_center[1] + (h1 / 2))]]
    ...
    for idx in range(len(placed_entities2)):
        ...
        current_center = [current_entitiy.center[0] + entity2_center[0] - int(w2 / 2),
                          current_entitiy.center[1] + entity2_center[1] - int(h2 / 2)]
        img, tmp_bbox = draw_textbox(self, img, current_entitiy, current_center)
    ...
    w2 = text2_shape[0]
    h2 = text2_shape[1]
    entity2_bbox = [[int(entity2_center[0] - (w2 / 2)), int(entity2_center[1] - (h2 / 2))],
                    [int(entity2_center[0] + (w2 / 2)), int(entity2_center[1] + (h2 / 2))]]
        ...
        for relation_idx in range(50):
            ...
            for idx in range(50):
                ...
                if check_slice(template_im, slice_shape, x_target, y_target, self.padding):
                    ...
                    entity1_center, entity2_center, entity_configuration, template_im = get_entity_placement(
                        slice_shape, x_target, y_target, (w1, h1), (w2, h2), template_im)

                    try:
                        template_im, relationship_bbox, placed_entities1, placed_entities2 = draw_relationship(self,
                                                                                                               template_im,
                                                                                                               entity1_center,
                                                                                                               entity2_center,
                                                                                                               entity_configuration,
